...
if (!(handlerType === 'h' || handlerType === 'rh' || handlerType === 'json' || handlerType === 'sql')) {
  console.log('usage: node http-client.js [h|rh|json|sql] [url]');
  process.exit(1);  
}
...
var db_handler = createResponseHandler(function (data) {
...
switch (handlerType) {
  ...
  case 'json':
    ...
    break;  
  case 'sql':
    var req = http.request(options, db_handler);
    req.end();
    break;
  default:
